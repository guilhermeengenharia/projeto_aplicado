    ...
    protected static apply(key: PropertyKey, descriptor: PropertyDescriptor, logLevel: LogLevel) {
        ...
        descriptor.value = function () {
            ...
            try {
                ...
                if (err instanceof Error) GenericLogDecorator.handlerMethodTrace(logTrace, start, logLevel, err);
                else GenericLogDecorator.handlerMethodTrace(logTrace, start, logLevel, new Error(String(err)));
                throw err;
            }
        };
        return descriptor;
